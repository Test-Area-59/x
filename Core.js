...
module.exports = async (Xtroid, m, commands, chatUpdate, store) => {
  try {
    ...
    const metadata = isGroup ? await Xtroid.groupMetadata(from) : {};
    const pushname = m.pushName; //|| 'NO name'
    const participants = isGroup ? metadata.participants : [sender];
    const groupAdmin = isGroup
      ? participants.filter((v) => v.admin !== null).map((v) => v.id)
      : [];
    const botNumber = await Xtroid.decodeJid(Xtroid.user.id);
    const isBotAdmin = m.isGroup ? groupAdmin.includes(Xtroid.user?.jid) : false;
    const isAdmin = isGroup ? groupAdmin.includes(sender) : false;
    const isCreator = [botNumber, ...global.owner]
      .map((v) => v.replace(/[^0-9]/g, "") + "@s.whatsapp.net")
      .includes(m.sender);
    const isOwner = global.owner.includes(m.sender);
    global.suppL = "t.me/animebotinfo";
    ...
    if (checkdata) {
      ...
      if (m.isGroup && mongoschema == "true") {
        linkgce = await Xtroid.groupInviteCode(from);
        if (budy.includes(`https://chat.whatsapp.com/${linkgce}`)) {
          ...
          kice = m.sender;
          await Xtroid.groupParticipantsUpdate(m.from, [kice], "remove");
        ...
        if (m.quoted) {
          if (m.quoted.sender == botNumber) {
            const botreply = await axios.get(
              `http://api.brainshop.ai/get?bid=173647&key=GqZxA2c6XSNbqZlg&uid=[uid]&msg=[${budy}]`
            );
    ...
    if (body.startsWith(prefix) && !icmd) {
      let Xtext = `No such command programmed *${pushname}* senpai! Type *${prefix}help* or press the button below to get my full command list!\n`;
      ...
      Xtroid.sendMessage(m.from, bmffg, {
        quoted: m,
      });
    }
    ...
    if (
      ...
      await Xtroid.sendMessage(m.from, reactionMess).then(() => {
        return Xtroid.sendMessage(m.from, buttonmess, {
          react: "🍁",
          quoted: m,
        });
      });
    ...
    if (cmd.react) {
      ...
      await Xtroid.sendMessage(m.from, reactm);
    }
    ...
    if (timestamps.has(m.sender)) {
      ...
      if (now < expiration) {
        ...
        return await Xtroid.sendMessage(
          m.from,
          {
